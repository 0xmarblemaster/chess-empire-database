...
// Debut branch Mon-Wed-Fri schedule: standard 1h slots for coach Nail Ildusovich
const ATTENDANCE_TIME_SLOTS_DEBUT_MON_WED_FRI_NAIL = [
    '9:00-10:00',
    '10:00-11:00',
    '11:00-12:00',
    '12:00-13:00',
...
// Debut branch Tuesday-Thursday schedule for coach Nail Ildusovich
const ATTENDANCE_TIME_SLOTS_DEBUT_TUE_THU_NAIL = [
    '9:00-10:00',
    '10:00-11:00',
    '11:00-12:00',
    '12:00-13:00',
...
// Gagarin Park Monday-Wednesday schedule for Coach Vasily Mikhaylovich (пн-ср)
const ATTENDANCE_TIME_SLOTS_GAGARIN_MON_WED_VASILY = [
    '9:00-10:00',
    '10:00-11:00',
    '11:00-12:00',
    '12:00-13:00',
...
function getTimeSlotsForBranch(branchName, scheduleType = null, coachName = null, monthKey = null) {
    ...
    if (normalizedName.includes('gagarin') || normalizedName.includes('гагарин')) {
        ...
        // Coach Vasily Mikhaylovich has custom slots for all schedules
    ...
    if (normalizedName.includes('debut') || normalizedName.includes('дебют')) {
        ...
        // Coach Nail Ildusovich has custom slots
        if (normalizedCoach.includes('nail') || normalizedCoach.includes('наиль')) {
            if (scheduleType === 'tue_thu') {
                return ATTENDANCE_TIME_SLOTS_DEBUT_TUE_THU_NAIL;
            }
            if (scheduleType === 'sat_sun') {
...
async function showAttendanceManagement(updateHash = true) {
    ...
    // coach_branches assignments — the dropdown defaults will pick a valid one.
    // The coach filter is resolved later, once the branch is settled, because
    // it depends on which branch we land on (single- vs multi-coach).
    ...
    // Branch is now settled — resolve the coach filter against it. Locked
    // coaches at multi-coach branches keep a saved peer; everywhere else this
    // falls back to the original lock / saved-value preservation.
    if (window.attendanceRoleLock) {
        if (window.attendanceRoleLock.isCoachLocked(roleInfo)) {
            attendanceCurrentCoach = window.attendanceRoleLock.resolveCoachFilterForBranch(
                roleInfo, window.coaches, attendanceCurrentBranch, attendanceCurrentCoach
            );
        } else {
...
function onAttendanceBranchChange() {
    ...
    // Reset coach filter to 'all' for admins; for locked coaches use the
    // branch-aware resolver — at single-coach branches it pins to self, at
    // multi-coach branches it preserves a previously-selected peer if they're
    // also at the new branch, otherwise defaults to self.
    if (window.attendanceRoleLock && window.attendanceRoleLock.isCoachLocked(attendanceRoleInfo)) {
        attendanceCurrentCoach = window.attendanceRoleLock.resolveCoachFilterForBranch(
            attendanceRoleInfo, window.coaches, attendanceCurrentBranch, attendanceCurrentCoach
        );
    } else {
...
function onMobileAttendanceBranchChange() {
    ...
    // Coach filter on branch change — see onAttendanceBranchChange for the full
    // rationale: admins reset to 'all'; locked coaches preserve a peer at the
    // new branch if possible, otherwise default to self.
    if (window.attendanceRoleLock && window.attendanceRoleLock.isCoachLocked(attendanceRoleInfo)) {
        attendanceCurrentCoach = window.attendanceRoleLock.resolveCoachFilterForBranch(
            attendanceRoleInfo, window.coaches, attendanceCurrentBranch, attendanceCurrentCoach
        );
...
// Shared HTML-escape helper used by ratings/uploads previews.
...
function _handleGuestRpcReason(data) {
    ...
    } else if (reason === 'no_razryad') {
        msg = _tt('admin.tournaments.noRazryad');
