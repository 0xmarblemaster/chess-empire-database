-- Migration: get_public_tournament_schedule RPC
--
-- Collapses the public tournament schedule page's 3 sequential Supabase
-- round-trips (branches, tournaments, registration_counts) into a single
-- RPC call. The function returns a JSONB document shaped as
--
--   { "branches": [ { "id", "name", "upcoming_tournaments": [...] }, ... ] }
--
-- Each tournament row preserves the column names tournaments.js already
-- renders (tournament_date, start_time, time_format, capacity, status,
-- registration_deadline) and adds a precomputed registration_count derived
-- from a single LEFT JOIN against tournament_registrations.
--
-- RLS already permits anon SELECT on both tournaments and
-- tournament_registrations (see migrations/043_tournaments.sql §3), so
-- SECURITY INVOKER is sufficient — no SECURITY DEFINER needed.
--
