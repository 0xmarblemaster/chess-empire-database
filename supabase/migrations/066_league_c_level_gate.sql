-- Migration 066: League C Level Gate
--
-- League C tournaments additionally require the registering student to be at
-- Level 2 or higher (students.current_level >= 2). Level 1 students are still
-- on fundamentals and must not enter League C events, even though their low
-- rating maps them into League C via calc_league_from_rating (migration 049).
--
-- Function body is based on the live definition after migration 051
-- (guest registrations + guest full name).
--
-- Semantics:
--   * Applies only when tournaments.league = 'C' and a student registers
--     (p_student_id IS NOT NULL). Guests / walk-ins bypass, same as 049.
--   * NULL current_level is allowed (unknown = verified off-system).
--   * Runs after the rating-league check, so 'ineligible' (wrong league)
--     takes precedence over 'level_too_low' (right league, too early).
--
-- New rejection reason in JSONB:
--   { ok: false, reason: 'level_too_low', student_level: <int>, required_level: 2 }
--
