...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-session-id, x-api-key',
}
...
serve(async (req) => {
  ...
  try {
    ...
    const supabaseAdmin = createClient(
      ...
      (Deno.env.get('CE_SECRET_KEY') ?? '') || (Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? ''),
      {
        auth: {
