 ...
 * ported to admin-v2.js — the file actually loaded by the deployed admin page.
 *
 * Parallel to tests/test-attendance-debut-slot-capacity.js, which guards the
 * same fix in admin.js (commit 97d45a5). admin-v2.js is the v2 attendance
 * implementation served in production; without this port, the Debut branch
 * stayed at 15 capacity even after the admin.js fix shipped.
 *
 * Covers:
 *   - Constants present with the expected values.
 *   - Helpers isDebutCurrentBranch / getDefaultRows / getMaxCapacity declared.
 ...
 *   - Static check: the rewired call sites in admin-v2.js no longer reference
 *     the raw constants — they go through the helpers (or the local
 *     selectedBranch check for the add-student form, which uses a different
 *     branch source).
 ...
 *
 * Run: node tests/test-attendance-debut-slot-capacity-v2.js
 */
...
// We rebuild the four declarations in a sandbox where
// `attendanceCurrentBranch` is a controllable variable. This mirrors how the
// helpers will see `attendanceCurrentBranch` at runtime — they read it from
// outer scope.
// ---------------------------------------------------------------------------
...
// raw constants. We assert by pattern presence in admin-v2.js.
// ---------------------------------------------------------------------------
...
// The auto-bucketing inside initializeStudentTimeSlots is gated by
...
// admin-v2.js uses `let slot = Math.floor(index / DEFAULT_TIME_SLOT_ROWS)`
// (the v2 multi-slot model writes to student.timeSlotIndexes).
assert(/Math\.floor\(\s*index\s*\/\s*DEFAULT_TIME_SLOT_ROWS\s*\)/.test(ADMIN_SRC),
...
// Cross-check: the rewired sites must not reference the raw constants.
